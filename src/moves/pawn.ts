...
export const getPawnPseudoLegalMoves = (
	...
	if (isOnBoard(singleForwardSq) && gameState.board[singleForwardSq] === null) {
		if (
			from0x88(singleForwardSq).rank ===
			PAWN_OFFSETS[pieceAtFrom.color].promotionRank
		) {
			pseudoLegalMoves.push(
				...generatePromotionMoves(fromSq, singleForwardSq, pieceAtFrom, null)
			)
		} else {
			pseudoLegalMoves.push({
				from: fromSq,
		...
		if (currentRank === PAWN_OFFSETS[pieceAtFrom.color].startingRank) {
			const doubleForwardSq =
				fromSq + PAWN_OFFSETS[pieceAtFrom.color].forward * 2
			if (
				isOnBoard(doubleForwardSq) &&
				gameState.board[doubleForwardSq] === null
			) {
				pseudoLegalMoves.push({
					from: fromSq,
					to: doubleForwardSq,
	...
	const captureOffsets = [
		PAWN_OFFSETS[pieceAtFrom.color].captureLeft,
		PAWN_OFFSETS[pieceAtFrom.color].captureRight,
	]
	...
	for (const offset of captureOffsets) {
		...
		if (isOnBoard(captureSq)) {
			...
			if (
				pieceAtCapture !== null &&
				pieceAtCapture.color !== pieceAtFrom.color
			) {
				if (
					from0x88(captureSq).rank ===
					PAWN_OFFSETS[pieceAtFrom.color].promotionRank
				) {
					pseudoLegalMoves.push(
	...
	if (gameState.enPassantTargetSquare !== null) {
		...
		const { file: enPassantFile, rank: enPassantRank } = from0x88(
		...
		if (pieceAtFrom.color === Color.White && enPassantRank === Rank.R6) {
			if (currentRank === Rank.R5 && enPassantRank === Rank.R6) {
				if (pawnFile + 1 === enPassantFile)
					potentialEnPassantCapture =
						fromSq + PAWN_OFFSETS[Color.White].captureRight
				if (pawnFile - 1 === enPassantFile)
					potentialEnPassantCapture =
						fromSq + PAWN_OFFSETS[Color.White].captureLeft
			}
		} else {
			if (currentRank === Rank.R4 && enPassantRank === Rank.R3) {
				if (pawnFile + 1 === enPassantFile)
					potentialEnPassantCapture =
						fromSq + PAWN_OFFSETS[Color.Black].captureRight
				if (pawnFile - 1 === enPassantFile)
					potentialEnPassantCapture =
						fromSq + PAWN_OFFSETS[Color.Black].captureLeft
			}
		}

		if (
			potentialEnPassantCapture !== null &&
			potentialEnPassantCapture === gameState.enPassantTargetSquare
		) {
			const capturedPawnSq =
				gameState.enPassantTargetSquare -
				PAWN_OFFSETS[pieceAtFrom.color].forward
			const capturedPawn = gameState.board[capturedPawnSq]

			if (
				capturedPawn &&
				capturedPawn.type === PieceType.Pawn &&
				capturedPawn.color !== pieceAtFrom.color
			) {
				pseudoLegalMoves.push({
					from: fromSq,
